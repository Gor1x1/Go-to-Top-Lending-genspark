...
- **Версия кода:** v0.6.0 (Phase 3C — CMS-интеграция подстраниц завершена, build PASS).
- **`CACHE_VERSION`:** **`v17`** (`src/lib/cache-config.ts`) — bump'нут в Phase 3C для сброса edge-кеша.
- **Финансовые баги BUG-001…BUG-009** из этого отчёта — закрыты (см. [`FIX_LOG.md`](FIX_LOG.md), март 2026); P&L каскад сходится.
- **Открытыми остаются** мелкие пункты BUG-010…BUG-019 (MEDIUM/LOW) — не блокируют прод.
- **Архитектурно** добавлен слой подстраниц с CMS-блоками (`<page>__<key>`); финансовая логика не затронута.

> Этот файл — исторический срез аудита. Текущие изменения и acceptance criteria см. в [`FIX_LOG.md`](FIX_LOG.md), реестр субагентов и обзор — в [`README.md`](README.md).
