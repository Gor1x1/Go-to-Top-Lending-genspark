...
CREATE TABLE IF NOT EXISTS site_text_overrides (
  ...
  text_am TEXT DEFAULT '',
  href TEXT DEFAULT '',
  updated_at DATETIME DEFAULT CURRENT_TIMESTAMP,
  UNIQUE(page, txt_id)
);

CREATE TABLE IF NOT EXISTS site_custom_blocks (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  page TEXT NOT NULL,
  position_after TEXT DEFAULT '',
  title_ru TEXT DEFAULT '',
  title_am TEXT DEFAULT '',
  text_ru TEXT DEFAULT '',
  text_am TEXT DEFAULT '',
  button_text_ru TEXT DEFAULT '',
  button_text_am TEXT DEFAULT '',
...
async function runLatestMigrations(db: D1Database): Promise<void> {
  ...
  // Phase 5.1 — site_text_overrides: per-element text overrides for the
  // inline visual editor. Allows admins to edit ANY data-ru/data-am text
  // (not just CMS-backed `tb()` calls) directly on the live site.
  // Keyed by (page, txt_id). txt_id is generated by editor.js based on
  // DOM order: e.g., "home__txt_42". `href` allows link URL overrides too.
  try { await db.prepare(`CREATE TABLE IF NOT EXISTS site_text_overrides (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    page TEXT NOT NULL,
    txt_id TEXT NOT NULL,
    text_ru TEXT DEFAULT '',
  ...
  try { await db.prepare(`CREATE INDEX IF NOT EXISTS idx_text_overrides_page ON site_text_overrides(page)`).run(); } catch {}
  // Phase 5.1 — site_custom_blocks: blocks added through the inline editor.
  // SSR loads these on every page render and inserts them after `position_after`
  // (a `data-block-key` value, e.g. "home__hero") or at the page bottom.
  try { await db.prepare(`CREATE TABLE IF NOT EXISTS site_custom_blocks (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    page TEXT NOT NULL,
    position_after TEXT DEFAULT '',
    title_ru TEXT DEFAULT '',
  ...
  try { await db.prepare(`CREATE INDEX IF NOT EXISTS idx_custom_blocks_page ON site_custom_blocks(page, sort_order)`).run(); } catch {}
