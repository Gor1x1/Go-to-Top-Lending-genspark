...
import { seedProBlocks } from '../../seed-data-pro'
...
api.put('/site-blocks/:id', authMiddleware, async (c) => {
  ...
  // For chrome blocks (shell__nav, shell__footer, shell__bottom, shell__floats,
  // shell__modal), wipe any matching rows from site_text_overrides. The block's
  // own texts_ru/texts_am is the single source of truth — leftover override rows
  // (e.g. from older editor.js versions that wrote shell__nav__N as overrides)
  // silently overwrite fresh CMS edits on every SSR pass.
  try {
    const blockRow = await db.prepare('SELECT block_key FROM site_blocks WHERE id=?').bind(id).first<any>();
    const blockKey = blockRow?.block_key as string | undefined;
    if (blockKey && blockKey.startsWith('shell__')) {
      const pattern = `${blockKey}__%`;
      await db.prepare(
        "DELETE FROM site_text_overrides WHERE txt_id LIKE ? OR txt_id LIKE ?"
      ).bind(pattern, `%__${blockKey}__%`).run();
    }
  } catch { /* best-effort cleanup; don't block save */ }
...
api.post('/site-blocks/import-from-site', authMiddleware, async (c) => {
  ...
  // ===== Phase 4 chrome + subpage seed (shared with /site-blocks/seed-pro) =====
  // Without this step, only legacy block_keys (nav, footer, hero, …) are written
  // — but the SSR shell (renderPageShell in landing.ts) reads `shell__nav`,
  // `shell__footer`, `shell__bottom`, `shell__floats`, `shell__modal`, plus
  // `home__*` / `calculator__*` / `package__chrome` / `blog__chrome`. If those
  // rows are missing, edits made in the inline editor for nav/footer labels
  // silently revert (the editor saves them as site_text_overrides, but SSR
  // skips overrides for shell__* by design). Calling seedProBlocks here makes
  // a single "Скачать данные с сайта" click populate everything in one go.
  // INSERT OR IGNORE → never overwrites admin edits made after a prior seed.
  let proSeeded = 0;
  let proError: string | undefined;
  try {
    const r = await seedProBlocks(db);
    proSeeded = r.inserted;
    proError = r.error;
  } catch (e: any) {
    proError = String(e?.message || e);
  }

  return c.json({ success: true, imported: sortIdx, tg_messages: tgSortIdx, pro_seeded: proSeeded, pro_error: proError });
});
...
api.post('/site-blocks/seed-pro', authMiddleware, async (c) => {
  try {
    const r = await seedProBlocks(c.env.DB);
    if (r.error) return c.json({ error: r.error }, 500);
    return c.json({ success: true, total: r.total, inserted: r.inserted });
  } catch (e: any) {
    return c.json({ error: String(e?.message || e) }, 500);
  }
});

// NOTE: the original inline PRO_BLOCKS array (~400 lines covering shell /
// home / calculator / package / blog defaults) was extracted to
// `src/seed-data-pro.ts`. Both POST /site-blocks/seed-pro and
// POST /site-blocks/import-from-site now share the same `seedProBlocks(db)`
// helper from there — keeps the inline editor's CMS path (shell__nav,
// shell__footer, etc.) populated after a one-click "Скачать данные с сайта".
