...
async function computePnlForPeriod(db: D1Database, periodKey: string) {
  ...
  // Auto-generate tax payment placeholders from rules (idempotent: only inserts if record doesn't exist yet)
  // NOTE: this INSERT on GET is intentional — it seeds placeholder records that the user can then adjust
  // The corresponding UPDATE further down keeps calculated amounts in sync
  try {
    const rules = await db.prepare('SELECT * FROM tax_rules WHERE is_active = 1').all();
    const month = parseInt(periodKey.split('-')[1]);
    for (const rule of (rules.results || []) as any[]) {
  ...
  // Sync auto-calculated tax amounts to DB, but only for unlocked periods
  // (locked periods have is_locked=1 in period_snapshots — those amounts should not be mutated)
  const isPeriodLocked = snap?.is_locked === 1;
  if (!isPeriodLocked) {
    for (const t of taxItems) {
      if (t.is_auto && t.tax_rate > 0 && t.id) {
        // Only update if the stored amount differs from calculated to avoid pointless writes
        const storedAmount = (taxesRaw.results || []).find((r: any) => r.id === t.id)?.amount ?? null;
        if (storedAmount === null || Math.abs(Number(storedAmount) - (t.amount || 0)) > 0.001) {
