...
- **Phase 3** — `renderNewHomePage` in `landing.ts` now also accepts an optional `landingPackages: Array<{slug, title_ru/am, description_ru/am, price_text_ru/am, cover_url, ...}>` and renders a 3-card `.nh-packages` section between `#services` and `#why-buyouts`. The data is read from `__SITE_DATA.landingPackages` (populated server-side via `/api/site-data` — see `implementer.md`).
- **Phase 3** — `renderPackagePage({lang, siteOrigin, pkg, otherPackages})` in `landing.ts` renders the detail page for `/package/:slug` (photo + bilingual title/description + price text + "Other packages" cross-link grid + Product/BreadcrumbList JSON-LD). The route handler (also in `landing.ts`) loads the pkg by slug and at most 4 other visible packages.
