...
**Деплой:** ✅ Production (gototopwb.ru)
...
**Спринт 1 — Умные ярлыки на текстах (SSR-шаблоны)**
- `src/routes/landing.ts` — Python-скриптом добавлено **188 атрибутов** `data-edit-key` и `data-edit-idx` к HTML-элементам, которые содержат `tb()`-вызовы (shell, home, calculator, package, buyouts, services, faq, contacts, referral, blog).
- Добавлены **16 атрибутов** `data-block-key` на контейнеры секций (`data-section-id` → `data-block-key`) для drag-and-drop.

**Спринт 2-6 — Inline Editor (`public/static/editor.js`, новый файл)**

Новый клиентский скрипт ~500 строк. Активируется автоматически только если в `localStorage` есть валидный токен администратора (`gtt_token`). Для обычных посетителей файл невидим — нет никаких изменений в UX.

Функциональность:
1. **Плавающая Admin Bar** (фиксирован внизу) — видна только адмнам. Содержит: «Режим редактирования», переключатель RU/AM, счётчик несохранённых изменений, кнопку «Сохранить всё», кнопку «Отменить», ссылку «Открыть Админку».
2. **Click-to-Edit тексты** — в режиме редактирования все `[data-edit-key]` элементы становятся `contenteditable`. Синяя пунктирная рамка обозначает редактируемый текст. Enter — завершает редактирование, Escape — откатывает. Изменения трекируются до нажатия «Сохранить всё».
3. **Двуязычное редактирование** — переключатель RU/AM переключает отображение и сохранение текстов отдельно в `texts_ru` / `texts_am`.
4. **Drag-and-drop блоков** — подгружает SortableJS с CDN при первом включении режима. Каждый блок (`[data-block-key]`) получает иконку-«ручку» ☰. После отпускания вызывает `POST /api/admin/site-blocks/reorder`.
5. **Управление блоками** — у каждого блока появляется тулбар (видим при наведении): скрыть/показать (👁), дублировать (⧉), удалить (🗑 с confirm + history snapshot).
6. **Добавить блок** — кнопки «+ Добавить блок здесь» между секциями. При клике — выпадающее меню из 5 шаблонов (hero_banner, for_whom, benefits, cta_strip, empty). Создаёт через `POST /api/admin/site-blocks`.
7. **Редактирование кнопок/ссылок** — элементы с `data-edit-type="button"` поддерживают popup-редактор текста и URL.
8. **Toast-уведомления** — при всех действиях появляются цветные уведомления (зелёный/красный/серый).
9. **Безопасность** — все API-вызовы используют `Authorization: Bearer {gtt_token}`. Без токена — никакого UI не отображается.

**Спринт 7 — Деплой**
- `src/routes/landing.ts` — добавлен `<script src="/static/editor.js?v=CACHE_VERSION" defer>` после `landing.js` в `renderPageShell` и в главном лендинге.
- `src/lib/cache-config.ts` — `CACHE_VERSION` поднята с `v28` до `v29`.

### Что НЕ тронуто
- Старый длинный лендинг (`/`) — работает как прежде; `data-edit-key` там нет, inline-редактор на него не влияет.
- `src/admin/panel.ts` — без изменений.
- База данных (D1) — без изменений.

### Как использовать
1. Войдите в `/admin` — JWT-токен сохраняется в `localStorage.gtt_token`.
2. Откройте любую страницу (`/home`, `/about`, `/services`, и т.д.).
3. Внизу появится Admin Bar. Нажмите «Режим редактирования».
4. Кликайте на тексты, редактируйте прямо на сайте.
5. Нажмите «Сохранить всё» — изменения уйдут в D1, кеш Cloudflare сброшен.
